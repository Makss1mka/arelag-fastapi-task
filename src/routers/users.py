...
from src.schemas.users import UserBalanceResponseModel, UserCreateRequestModel, UserResponseModel, UserUpdateRequestModel


user_router = APIRouter()


@user_router.get("/users", response_model=List[UserResponseModel], status_code=status.HTTP_200_OK)
async def get_users(
...
@user_router.post("/users", status_code=status.HTTP_200_OK)
async def post_user(
...
@user_router.patch("/users/{user_id}", response_model=typing.Optional[UserModel] | None)
